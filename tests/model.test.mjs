...
test('popup width follows enabled providers and remains usable for settings',()=>{assert.ok(popupWidth(3)<popupWidth(8));assert.equal(popupWidth(8)-popupWidth(7),90);assert.ok(popupWidth(0)>=240);assert.equal(popupWidth(1,true),popupWidth(8,true));});
test('opening provider details preserves the compact overview width',()=>{assert.equal(popupWidth(3),294);for(let count=0;count<=PROVIDERS.length;count++){assert.equal(popupWidth(count,false,true),popupWidth(count,false,false));assert.equal(popupWidth(count,true,true),popupWidth(count,true,false));}});
