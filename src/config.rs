...
pub const RING_COLORS: [&str; 8] = ["urgency","provider","teal","blue","violet","rose","amber","green"];
pub const LAYOUTS: [&str; 2] = ["grid","stack"];
pub fn config_path() -> PathBuf { dirs::config_dir().unwrap_or_else(|| PathBuf::from(".")).join("TokenTray").join("config.json") }
#[derive(Clone, Serialize, Deserialize)]
#[serde(default)]
pub struct Settings { pub disabled: BTreeSet<String>, pub acrylic: bool, pub start_minimized: bool, pub ring_color: String, pub layout: String, pub onboarded: bool }
impl Default for Settings { fn default() -> Self { Self { disabled: BTreeSet::new(), acrylic: true, start_minimized: true, ring_color: "urgency".into(), layout: "grid".into(), onboarded: false } } }
impl Settings {
    pub fn enabled(&self, id: &str) -> bool { PROVIDERS.contains(&id) && !self.disabled.contains(id) }
    pub fn valid(&self) -> bool { self.disabled.iter().all(|id| PROVIDERS.contains(&id.as_str())) && RING_COLORS.contains(&self.ring_color.as_str()) && LAYOUTS.contains(&self.layout.as_str()) }
    ...
    pub fn save(&self) -> Result<(), String> {
        ...
        if !LAYOUTS.contains(&self.layout.as_str()) { return Err("Unknown layout".into()); }
...
#[cfg(test)] mod tests {
    ...
    #[test] fn ring_color_defaults_to_urgency_and_survives_round_trip() { let s: Settings = serde_json::from_str("{}").unwrap(); assert_eq!(s.ring_color, "urgency"); assert!(s.valid()); let mut s = Settings::default(); s.ring_color = "amber".into(); let restored: Settings = serde_json::from_slice(&serde_json::to_vec(&s).unwrap()).unwrap(); assert_eq!(restored.ring_color, "amber"); }
    #[test] fn layout_defaults_to_grid_and_rejects_unknown_values() { let s: Settings = serde_json::from_str("{}").unwrap(); assert_eq!(s.layout, "grid"); assert!(!s.onboarded, "a fresh profile has not been through first run"); let mut s = Settings::default(); s.layout = "stack".into(); s.onboarded = true; let restored: Settings = serde_json::from_slice(&serde_json::to_vec(&s).unwrap()).unwrap(); assert_eq!(restored.layout, "stack"); assert!(restored.onboarded); let mut s = Settings::default(); s.layout = "carousel".into(); assert!(!s.valid()); assert!(s.save().is_err()); }
