...
Entering a page or revealing details uses a brief fade with a small directional offset so changes do not cut instantly; background readings never replay it. Requesting a refresh turns every ring one full turn, and a reading that lands on its own — the five-minute poll, startup, a provider switched on — turns just its provider's ring; arrivals that answer a manual request do not turn again. Either way the ring settles back to the same angle, and provider cooldowns still decide whether a read follows. Reduced-motion preferences keep every transition instant, and motion never delays window resizing.
